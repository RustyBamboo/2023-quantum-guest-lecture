...
# ╔═╡ 7ae2c188-810a-11ee-198b-f7366053c5cb
using LinearAlgebra, Manifolds, Zygote, GLMakie, MeshIO, GeometryBasics, FileIO

# ╔═╡ 2fda6fce-09a1-464a-b89e-fb1f30b4023e
resolution = (800,800)

# ╔═╡ 9360fe7b-8214-43a9-ab11-c7ed2d145577
# Load the STL file
m = load("../sub8.stl")

# ╔═╡ 1f562be7-31ce-406a-9078-3df3d7dbc00c
function to_axis_angle(R)
	θ = acos(0.5 *(tr(R) - 1))
	ω = 1/(2*sin(θ)) * [R[3,2] - R[2,3] R[1,3] - R[3,1] R[2,1] - R[1,2]]
	return θ, Vec3f(vec(ω))
end

# ╔═╡ c4ed05d0-4abb-489f-b08f-d0638cac8e3c
function to_axis_angle_grads(g)
	v = [-g[2,3], g[1,3], -g[1,2]]
	a = norm(v)
...
# ╔═╡ 905c703c-5e55-4f98-a6b9-2006cca39a36
	function visualize_rotation(SE; name="../images/so_3_optim.gif")
		fig = Figure(resolution = resolution)
	
		
		pos = Observable([Point3f(0.0,0.0,0.0)])
		
	 	x_rotated = Observable(Point3f[])
		y_rotated = Observable(Point3f[])
		z_rotated = Observable(Point3f[])
		display(x_rotated)
		size = 1
		ax = Axis3(fig[1,1], aspect=(1,1,1), viewmode=:fit, limits = (-size, size, -size, size, -size, size))
	
		arrows!(ax, pos, x_rotated,arrowsize=0.2,color=:red)
		arrows!(ax, pos, y_rotated,arrowsize=0.2,color=:green)
		arrows!(ax, pos, z_rotated,arrowsize=0.2,color=:blue)

		scene = mesh!(ax, m)

		angle = Observable(0.0) 
		axis = Observable(Vec3f(0, 0, 1))
		

		# GLMakie.rotate!(scene, axis, angle)
		lift((a, ax) -> GLMakie.rotate!(scene, ax, a), angle, axis)
	
		record(fig, name, 1:length(SE)) do frame
			# empty!(fig)
			R = SE[frame][begin:3,begin:3]
			# t = SE[frame][begin:3,end]
	   		 # Rotated coordinate frame
	
			
				
		 	x_rotated[] = [Point3f(R * [1.0; 0.0; 0.0])]
			y_rotated[] = [Point3f(R * [0.0; 1.0; 0.0])]
			z_rotated[] = [Point3f(R * [0.0; 0.0; 1.0])]
			...
			# scene = mesh!(ax, m)
			# GLMakie.rotate!(scene, Vec3f(1, 0, 0), 0.2 * frame)

			a,b = to_axis_angle(SE[frame])
			angle[] = a
			axis[] = b
...
# ╔═╡ 1c1d2ac6-756b-4c7a-8bdf-6fc8231000ec
function visualize_rotation_vec(SE, velocity; name="../images/so_3_optim_vec.gif")
		fig = Figure(resolution = resolution)

		v_vector = Observable([Point3f(to_axis_angle_grads(velocity[1])[2])])
		...
		arrows!(ax, pos, v_vector, arrowsize=0.1, color=:black)
		...
		record(fig, name, 1:length(SE)) do frame
			...
			v_vector[] = [Point3f(to_axis_angle_grads(velocity[frame])[2])]
...
# ╔═╡ c5f9600e-9353-45a6-9132-0f4deec6cd14
M = SpecialOrthogonal(3)

# ╔═╡ 4302b116-b247-49ef-bc9e-8d4952df8a57
X_mat = rand(M)

# ╔═╡ ec5d0ad3-e904-4941-b50a-60d8f80c45a7
function loss(X)
	# R = X[begin:3,begin:3]
	# t= X[begin:3,end]
	# θ,_ = to_axis_angle(X_mat[begin:3,begin:3])
	# return sqrt(2) * abs(θ) + norm(t)
	# return tr(X)
	# return log(X)
	# 1 - tr(R) + norm(t)
	return norm(X-I(3))
...
# ╔═╡ b105f875-3a43-43ab-b8c5-a9d801989434
grad = Zygote.gradient(loss,X_mat)[1]

# ╔═╡ bce3e9ae-67b3-4f52-b1d4-dcf7283d9362
function optim_adam(X, loss, N=100,α = 0.001, β1 = 0.9, β2 = 0.999, ε = 1e-8)
    history = [loss(X)]
    path = []
	grads = []

    m = zero(X)      # First moment estimate (Moving average of the gradient)
    v = 1      # Second moment estimate (Moving average of the squared gradient)
    t = 0            # Initialize timestep

    for _ in 1:N
        t += 1
        grad = Zygote.gradient(loss, X)[1]
		grad = grad / norm(grad)
        ...
        # Compute update step
        step = -α * m_hat ./ (sqrt(v_hat) + ε)

		step = project(M, X, step)
        
        # Normalize and retract
        # step = -step / norm(step)
        X = retract(M, X, step)
    ...
    return X, history, path, grads
end

# ╔═╡ e7b579b6-3602-403a-9224-fe444687e423
X_opt, history,path, grads = optim_adam(X_mat, loss, 150,1.5,0.9,0.99995)

# ╔═╡ 6b4c5cba-8e3c-4fbe-8244-6bfe65c6c84f
length(path), length(grads)

# ╔═╡ 8736a758-3668-4504-b1a5-9d66d8558a7c
a,b = to_axis_angle(path[1])

# ╔═╡ 4cd75378-c4ee-48ff-9eb8-5f3a030d0929
plot(history)

# ╔═╡ 9d3f8773-2ce1-4dab-b4f7-24623e459836
fig_o = visualize_rotation(path)

# ╔═╡ 4086d690-247d-4f75-9a31-5a9d7caf6f59
visualize_rotation_vec(path, grads; name="../images/so_3_optim_vec.gif")

# ╔═╡ 6797ba0a-8849-4472-be9d-385305d02683
length(path), length(grads)

# ╔═╡ 28bd667b-d1fd-4143-a30a-78181dfd04ab
function evolve(X, grads)
	path = []
	for g in grads
		push!(path,X)
		X = retract(M, X, g)
...
# ╔═╡ eb3bad8e-25d8-4732-a91f-024cbee1f6df
begin
	# path_1 = copy(path)
	# path_1[1] = rand(M)
	visualize_rotation(evolve(rand(M), grads); name="../images/uncertain_state.gif")
end

# ╔═╡ 26e1265b-814d-438c-ba5b-65df3a0a96bb
begin

	rand_t = [rand(M; vector_at=x) for x in path]
	new_grad = grads .+ 0.05*rand_t
visualize_rotation_vec(evolve(path[1], new_grad), new_grad; name="../images/pertubation.gif")
end

# ╔═╡ 77f65eee-71d8-4c24-82f3-e03f5f0f905f
...
# ╔═╡ 32af1b99-98e9-460f-b313-5defb46faafa
function extract_rotation_axis(S)
    return [S[3,2], S[1,3], S[2,1]]
end

# ╔═╡ e2f050f4-06b2-4d7f-87dd-366537bc9872
# Function to plot quaternions on a sphere
function plot_quaternions_on_sphere(rotations)
	...
	ax2 = Axis(fig[1,2], aspect=1, yreversed=true)

	points = Observable(Point3f[]) 
	scatter!(ax, points,color = :blue, markersize = 5, strokewidth=0,  transparency = true)
	sphere = GLMakie.Sphere(Point3f0(0), 1)
    mesh!(ax, sphere, color = :gray)

	rot = Observable(rotations[1])
	
	heatmap!(ax2, rot, colorrange=(-1,1))

	
	record(fig, "../images/rotations.gif", 1:length(quaternions)) do frame
			q = quaternions[frame]
			x = 2 * (q[2]*q[4] + q[1]*q[3])
        	y = 2 * (q[3]*q[4] - q[1]*q[2])
        	z = 1 - 2 * (q[2]^2 + q[3]^2)
...
# ╔═╡ 9d431587-28ee-4b36-8931-4889b0da11b6
plot_quaternions_on_sphere(path)

# ╔═╡ f75d74b9-216e-48ba-9561-932a6729a135
path[1]
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "4d7302d2a1f155cc7f6fc7fa8b1ba9f7191bc270"
...
git-tree-sha1 = "81bac5bb2ae3ea7394d96dac94f3c2c587b81d8d"
...
git-tree-sha1 = "bed8e5ec7658929056781663d18d4bfaf416f50b"
...
[[deps.ManifoldsBase]]
deps = ["LinearAlgebra", "Markdown", "Random", "Requires"]
git-tree-sha1 = "e43ebe65e1313b5dcc8e7a84a62941955568a4d2"
uuid = "3362f125-f0bb-47a3-aa74-596ffd7ef2fb"
version = "0.15.2"
weakdeps = ["RecursiveArrayTools"]
